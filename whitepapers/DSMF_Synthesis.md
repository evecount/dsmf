...
*   **Correlation Aperture ($\beta$):** A dynamic "zoom lens" adjusting the resolution and strength of observed correlations at each recursive level. Mathematically, $\beta$ directly multiplies the classical input $z$ at each level of recursion: $\sigma^n(z, |\psi\rangle, |B\rangle) = \sigma(D^\alpha(\sigma^{n-1}(\beta z, |\psi\rangle, |B\rangle)))$. By scaling the input $z$ at every nested layer, the $\beta$ parameter reveals the finer details of the micro-sigmoids embedded within the larger main sigmoid, establishing a self-similar, multi-scale structure that bridges macro, meso, and micro levels of information. Beyond acting as a simple mathematical scaling factor, $\beta$ functions as a correlation tracker—much like a financial beta index—measuring how strongly different variables move together, scaling the micro-sigmoids to adjust the resolution and strength of simultaneous correlations. In physical manifestations, such as photonic quantum systems, the $\beta$ aperture literally scales the micro-structures by physically controlling the width of light beams and the degree of quantum state correlation.
