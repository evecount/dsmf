...
*   **Mathematical Role:** $\alpha$ represents the fractional derivative order ($D^\alpha$). It captures the compression and decompression dynamics of a wave function, analogous to those observed during quantum tunneling. Because fractional calculus incorporates non-local kernels, $\alpha$ is mathematically defined by its capacity to capture the "memory" and non-local history of a continuous wave trajectory.
